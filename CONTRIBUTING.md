# Building
## Running & Debug
- If building using managed IDEs like Rider or Visual Studio, ensure that a valid `--runtime ...` argument is being passed to `dotnet`, or `RuntimeIdentifier=...` is set for calling `msbuild`. This is required for runtime-specific resources to be included in the build. AuraFlow currently supports building for the `win-x64`, `linux-x64` and `osx-arm64` runtimes.
- You can also build the `AuraFlow.Avalonia` project using `dotnet`:
```bash
dotnet build ./AuraFlow.Avalonia/AuraFlow.Avalonia.csproj -r win-x64 -c Debug
```
- Note that on Windows, the `net8.0-windows10.0.17763.0` framework is used, build outputs will be in `AuraFlow.Avalonia/bin/Debug/net8.0-windows10.0.17763.0/win-x64`. On other platforms the `net8.0` framework is used.
...
### Windows
```bash
dotnet publish ./AuraFlow.Avalonia/AuraFlow.Avalonia.csproj -r win-x64 -c Release -p:Version=$env:RELEASE_VERSION -p:PublishSingleFile=true -p:IncludeNativeLibrariesForSelfExtract=true -p:PublishReadyToRun=true
```
### macOS
...
- Building the `AuraFlow.Avalonia` project once should also install Husky.Net, or run the following command:
```bash
